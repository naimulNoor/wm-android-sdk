...
class ImageCompresser(val height:Int,val width:Int) {
    ...
    private val dimensionHeight=500
    private val dimensionWidth=500

    companion object {
        val instance = ImageCompresser(height = 500, width = 500)
    }
    ...
    fun getCompressedImageFile(imagePath: String, mContext: Context): File {
        ...
        try {
            ...
            scaledBitmap = Bitmap.createBitmap(scaledBitmap, 0, 0, this.width, this.height, matrix, true)
        } catch (e: IOException) {
            e.printStackTrace()
        }
        ...
        val file = File(getFilename(mContext))
        try {
    ...
    fun getFilename(mContext: Context): String {
        val mediaStorageDir = File(
            "${mContext.getExternalFilesDir(Environment.DIRECTORY_PICTURES)}/Android/data/" + BuildConfig.APPLICATION_ID + "/files_root"
        )
