...
class ZoomableImageView(context: Context, attr: AttributeSet?) : androidx.appcompat.widget.AppCompatImageView(context, attr) {
    ...
    private inner class ScaleListener : SimpleOnScaleGestureListener() {
        ...
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            ...
            if (origWidth * saveScale <= width || origHeight * saveScale <= height) {
                matrixx.postScale(mScaleFactor, mScaleFactor, width / 2, height / 2)
                if (mScaleFactor < 1) {
                    matrixx.getValues(m)
                    ...
                    if (mScaleFactor < 1) {
                        if (Math.round(origWidth * saveScale) < width) {
                            if (y < -bottom) matrixx.postTranslate(
                                0f,
                                -(y + bottom)
                            ) else if (y > 0) matrixx.postTranslate(0f, -y)
                        } else {
                            if (x < -right) matrixx.postTranslate(
                                -(x + right),
                                0f
                            ) else if (x > 0) matrixx.postTranslate(-x, 0f)
                        }
                    }
                }
            } else {
                matrixx.postScale(mScaleFactor, mScaleFactor, detector.focusX, detector.focusY)
                matrixx.getValues(m)
                val x = m[Matrix.MTRANS_X]
                val y = m[Matrix.MTRANS_Y]
                if (mScaleFactor < 1) {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        redundantYSpace /= 2f
        redundantXSpace /= 2f
        matrixx.postTranslate(redundantXSpace, redundantYSpace)
